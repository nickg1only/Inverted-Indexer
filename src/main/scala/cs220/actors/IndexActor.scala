...
class IndexActor extends Actor with ActorLogging {
  ...
  def receive = {
	...
	val urlsInDocs = sql"""select url from documents where url = ${url}""".map(rs => rs.string("url")).list.apply()
	 if(!urlsInDocs.isEmpty()){
         sql"""insert into documents (url) values (${url})""".update.apply()
    ...
    case Word(url, word){
	 if(word.length < 50){
         val docID = sql"""select docid from documents where url = $url""".map(rs => rs.int("docid")).single.apply().get()
         theWordInWords = sql"""select word from words where word = ${word}""".map(rs => rs.string("word")).list.apply()
         if(!theWordInWords.isEmpty()){
           val wordID = sql"""select wordid from words where word = ${word}""".map(rs => rs.int("wordid")).single.apply().get()
           sql"""insert into index (wordid,docid) values (${wordID},${docID})""".update.apply()
         }
         else{
           sql"""insert into words (word) values (${word})""".update.apply()
           var wordid = sql"""select wordid from words where word = ${word}""".map(rs => rs.int("wordid")).single.apply().get()
           sql"""insert into index (wordid,docid) values (${wordID},${docID})""".update.apply()
         }
	}
    }
